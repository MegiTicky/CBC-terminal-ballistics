...
public record ImpactMark(ImpactMarkKind kind, TBCaliber caliber, Direction face, float x, float y, float z, float rotation, long gameTime) {
    ...
    public static ImpactMark load(CompoundTag tag) {
        ...
        return new ImpactMark(kinds[k], calibers[c], Direction.from3DDataValue(tag.getByte("Face")), tag.getFloat("X"), tag.getFloat("Y"), tag.getFloat("Z"), rotation, tag.getLong("Time"));
    }
